...
function discoverSharpBinaries(platformName) {
  ...
  for (const sharpArch of sharpArches) {
    const pkgRoot = path.join(ROOT, 'node_modules', '@img', `sharp-${sharpArch}`);
    const vipsRoot = path.join(ROOT, 'node_modules', '@img', `sharp-libvips-${sharpArch}`);
    const sharpRoot = path.join(ROOT, 'node_modules', 'sharp');

    // 1. Copy the main sharp package (JS parts)
    ...
    // 3. Copy the platform-specific libvips package (if separate)
    if (fs.existsSync(vipsRoot)) {
      NATIVE_ADDON_DIRS.push({ src: vipsRoot, dest: `node_modules/@img/sharp-libvips-${sharpArch}` });
    }
  }
}
...
function buildAll() {
  ...
  // ── 0. Bundling with ncc (resolves all JS deps except sharp) ────────────────
  log('Bundling with ncc…');
  const nccOut = path.join(DIST, 'ncc');
  ensureDir(nccOut);
  ...
  for (const target of activeTargets) {
    ...
    const pkgCmd = [
      ...
      `--output "${exeDest}"`,
      '--compress GZip',
      '--public', // Helps with some dynamic requires inside the bundle
    ].join(' ');
    ...
    for (const { src, dest } of NATIVE_ADDON_DIRS) {
      ...
      if (fs.lstatSync(src).isDirectory()) {
        copyDir(src, fullDest);
      } else {
        copyFile(src, fullDest);
      }
    }
