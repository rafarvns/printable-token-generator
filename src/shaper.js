 ...
 * Aplica o formato circular e o anel ao buffer da imagem
 */
async function applyTokenRing(imageBuffer, size, force = false) {
  const ringsConf = config.TOKEN_RINGS || {};
  if (!ringsConf.ENABLED && !force) return imageBuffer;

  try {
    // 1. Recorta a imagem base em círculo (com 1px de folga para não vazar pelas bordas do anel)
    const mask = createCircleMask(size - 2); 
